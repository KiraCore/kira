...
if [ ! -f "$EXECUTED_CHECK" ]; then
  ...
  if [ ! -f "$COMMON_GENESIS" ] ; then
    ...
    set +x 
    sekaid add-genesis-account $(sekaid keys show validator -a --keyring-backend=test --home=$SEKAID_HOME) 299999600000000ukex,299999600000000utest,2000000000000000samolean --home=$SEKAID_HOME
    ...
    sekaid add-genesis-account $(sekaid keys show faucet -a --keyring-backend=test --home=$SEKAID_HOME) 100000000ukex,100000000utest,100000000000000samolean --home=$SEKAID_HOME

...
while : ; do
    ...
    for addr in $(echo $ADDR_LIST | sed "s/,/ /g") ; do
        ...
        if [ ! -z "${dnsStandalone}" ] ; then
            ...
            snapshot_node_id=$(tmconnect id --address="$dns:$KIRA_SNAPSHOT_P2P_PORT" --node_key="$KIRA_SECRETS/seed_node_key.json" --timeout=3 || echo "")
            ...
            if ($(isNodeId "$snapshot_node_id")) && timeout 1 nc -z $dns $KIRA_SNAPSHOT_P2P_PORT ; then 
                tmp_addr="${snapshot_node_id}@${dns}:$KIRA_SNAPSHOT_P2P_PORT"
                [ -z "$DETECTED_NODES" ] && DETECTED_NODES="$tmp_addr" || DETECTED_NODES="${DETECTED_NODES},$tmp_addr"
                echoInfo "INFO: Port $KIRA_SNAPSHOT_P2P_PORT is exposed" ; 
            else 
                echoInfo "INFO: Port $KIRA_SNAPSHOT_P2P_PORT is NOT accepting P2P connections" ; 
            fi

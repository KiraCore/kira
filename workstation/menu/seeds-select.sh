...
while : ; do
    echoWarn "WARNING: If you want to connect to external networks you have to specify at least one public or private seed node"
    echoInfo "INFO: If you are launching a new network you should wipe entire content of the public and private seed nodes list"
    ...
    if [ "${TVAL,,}" == "v" ] ; then
        ...
        if [[ -z $(grep '[^[:space:]]' $PRIVATE_SEEDS) ]] && [[ -z $(grep '[^[:space:]]' $PUBLIC_SEEDS) ]] ; then

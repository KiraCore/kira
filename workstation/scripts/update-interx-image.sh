...
INTERX_IMAGE_EXISTS=$($WORKSTATION_SCRIPTS/image-updated.sh "$KIRA_DOCKER/interx" "interx" "latest" "$INTERX_INTEGRITY" || echo "error")
if [ "$INTERX_IMAGE_EXISTS" == "False" ]; then
    echo "All images were updated, starting interx image..."
    $WORKSTATION_SCRIPTS/update-image.sh "$KIRA_DOCKER/interx" "interx" "latest" "$INTERX_INTEGRITY" "REPO=$INTERX_REPO" "BRANCH=$INTERX_BRANCH" #4

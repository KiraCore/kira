...
if [ "$MASTER_MNEMONIC" == "autogen" ] ; then
    ...
    LOCAL_ENTROPY=$(openssl rand -hex 1024 | sha256)
    MASTER_MNEMONIC_1="$(bip39gen mnemonic --length=24 --entropy="$LOCAL_ENTROPY" --hex=true --verbose=false)"
    MASTER_MNEMONIC_2="$(bip39gen mnemonic --length=24 --entropy="$LOCAL_ENTROPY" --hex=true --verbose=false)"
